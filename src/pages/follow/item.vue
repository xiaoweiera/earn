        ...
        <div class="mt-2 flex items-center">
          ...
          <span v-if="data['nftList'].length>0" class="blue-text mr-1.5 whitespace-nowrap flex items-center">
              <template v-for="item in data['nftList']">
                <v-router :href="item['nftDetailUrl']" class="cursor-pointer">#{{ item['nftId'] }}<span
                    class="mx-1">/</span></v-router>
              </template>
           </span>
          ...
          <span v-if="data['eventType']!=='mint'"
                class="small-text">{{ getPrice() }} {{ _.toUpper(data['currency']) }}</span>
