...
<template>
  <div class="text-global-white">
    ...
    <div class="showX min-h-100 text-global-white relative" :key="key">
      <el-table v-if="list.length>0" :data="list" class="w-full table-fixed huiClass" :row-style="rowClass"
                :header-cell-style="headerCellClass" :cell-style="cellClass">
          ...
          <el-table-column :fixed="index === 0" :width="header.width ? header.width : 150">
            <template #header>
              <div class="relative h-full flex items-center" :class="{ 'justify-center': header.center }">
                <ui-sort class="header-name fit" :active="header.active" :sort="header.sort" :sort-data="param"
                         :key-name="header.sortField" :field="header.title" :name="getTitleCnEn(header)"
                         @change="sort"/>
            ...
            <template #default="scope">
              <div>
                <Content :center="header.center" :width="header.width" :fields="header.fields" :type="header.type"
                         :data="scope.row"/>
              </div>
            </template>
          </el-table-column>
        ...
        <el-table-column class-name="lie">
          ...
          <template #default="scope">
            ...
            <div class="flex items-center" :key="followKey">
              ...
              <div v-if="!isFollow(scope.row.addressName)" class="follow" @click="add(scope.row.addressName)">Follow
              </div>
