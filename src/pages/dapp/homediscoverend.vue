...
<template>
  <div class="mt-5 p-4 bg-global-white rounded-md">
    ...
    <div class="border-b-1 border-global-highTitle border-opacity-6 pb-4">
      <DappHomeHeader :status="Status.ended" :type="urlType"
                      tips="Data supported by 14 public chain and 65 platforms" title="Ended IDO & IGO Projects"/>
    </div>
    <!-- 项目类型、公链、搜索框 -->
    <div class="flex justify-between items-center mt-4">
      <div class="flex items-center">
        ...
        <DappDiscoversContentType v-if="summary.ixo" :key="key"
                                  :list="tabChain(summary.ixo.chain, 'bracket', config.home)" active-name="bracket"
                                  name="bracket" title="公链"></DappDiscoversContentType>
        <span class="h-6 border-l-1 border-global-highTitle border-opacity-10 mx-4"></span>
        <!-- 类型 -->
        <DappDiscoversContentChain v-if="summary.ixo" :key="key" :chainData="summary.ixo.chain" :href="config.home"
                                   name="category" title="类型"></DappDiscoversContentChain>
      </div>
      <!-- 搜索框 -->
      <div>
    ...
    <div class="mt-4 border-b-1 border-global-highTitle border-opacity-6 pb-4">
      <DappDiscoversContentType v-if="summary.ixo" :key="key"
                                :list="tabPlat(summary.ixo.platform, 'platform', config.home)" active-name="platform"
                                name="platform" title="platform"></DappDiscoversContentType>
    </div>
    <div>
      <DappDiscoversEndlist :key="key" :list="EndedList" :params="params"
                            @changeSort="changeSort"></DappDiscoversEndlist>
    </div>
  </div>
</template>
