...
import { formatCash, dateYMDFormat, isAfter, getNotEmptySize } from "src/utils/";
...
const isFooterEmpty = function (data: ProjectItem): boolean {
  const list = [data.mint_price, data.ido_fundraising_goal, data.ido_start_at, data.ido_end_at];
...
<template>
  <div>
    <div class="flex items-center justify-between">
      <Price :progress="data.mint_status" label="Status" value="Mint" />
      <div>
        <div v-if="isAfter(data.mint_start_at)" class="p-1">
          <ui-time-blue :value="data.mint_start_at" />
        </div>
        <div v-else-if="isAfter(data.mint_end_at)" class="p-1">
          <ui-time-blue :value="data.mint_end_at" />
        </div>
    ...
    <Table class="mt-4">
      ...
      <template v-else>
        ...
        <!--Mint Supply-->
        <Td v-if="data.issue_volume">
          <label class="text-12-18 text-global-highTitle text-opacity-65">{{ i18n.dapp.info.supply }}</label>
          <p class="text-14-18 text-global-highTitle">
            <b class="font-m">{{ formatCash(data.issue_volume) }}</b>
          </p>
        </Td>

        <!--Date-->
        <Td v-if="data.mint_start_at || data.mint_end_at">
          <label class="text-12-18 text-global-highTitle text-opacity-65">Mint Time (UTC)</label>
          <p class="text-14-18 text-global-highTitle">
            <b v-if="isAfter(data.mint_start_at)" class="font-m">
              {{ dateYMDFormat(data.mint_start_at) }}
            </b>
            <b v-else class="font-m">
              {{ dateYMDFormat(data.mint_end_at) }}
            </b>
          </p>
        </Td>
      </template>
