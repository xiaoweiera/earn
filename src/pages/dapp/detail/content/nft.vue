    ...
    <div class="mt-3 p-4 bg-global-topBg rounded-md">
      ...
      <div class="flex items-center justify-between">
        <div class="flex flex-col md:flex-row">
          <div class="flex items-center">
            <DAppDetailState :start="data.mint_start_at" :ended="data.mint_end_at" />
          ...
          <div class="flex items-center mt-1.5 md:mt-0">
            <p v-if="data.mint_start_at || data.mint_end_at" class="hidden md:block h-4 mx-3 border-l border-sold border-global-highTitle border-opacity-6"></p>
            <p class="flex items-center">
              <IconFont type="icon-rili" class="text-global-highTitle text-opacity-45" size="16" />
              <span class="ml-3 text-kd14px18px text-global-highTitle text-opacity-85 font-kdFang">{{ timeValue(data.mint_start_at, data.mint_end_at) }}</span>
            </p>
          </div>
        </div>
      ...
      <div class="flex flex-col md:flex-row justify-between overflow-hidden" :class="show_content ? 'h-0' : 'mt-5'">
        ...
        <div class="text-global-highTitle text-kd14px24px font-normal">
          <ui-description class="airdrop-des" :line="isAfter(data.mint_start_at) || isAfter(data.mint_end_at) ? 10 : 6">
            <ui-markdown :value="data.participation" />
          </ui-description>
        </div>
        <!-- 倒计时/数据/按钮 -->
        <div class="flex flex-col md:flex-row" :class="isAfter(data.mint_start_at) || isAfter(data.mint_end_at) ? 'md:h-65' : 'md:h-36'">
          <!-- 间隔线 -->
          ...
          <!-- 倒计时 -->
          <div>
            <DAppDetailDate :start="data.mint_start_at" :ended="data.mint_end_at" />
            <!-- 间隔线 -->
            <div v-if="isAfter(data.mint_start_at) || isAfter(data.mint_end_at)" class="w-87 my-5 border-b border-sold border-global-highTitle border-opacity-6"></div>
            <!-- 数据 -->
            <ul class="w-full md:w-87">
              <li class="flex justify-between items-center">
                ...
                <p class="airdrop-num">
                  <b class="airdrop-num">{{ toNumberFormat(data.mint_price) }}</b>
                  <span v-if="data.price_unit" class="airdrop-num ml-0.5">{{ toUpper(data.price_unit) }}</span>
                </p>
              </li>
              <li class="flex justify-between items-center mt-3">
                ...
                <p class="airdrop-num">
                  <span>{{ toNumberFormat(data.issue_volume) }}</span>
                </p>
              </li>
              <li class="flex justify-between items-center mt-3">
                <!-- Mint时间 -->
                ...
                <p class="airdrop-num">
                  <span>{{ timeFormat(data.mint_start_at, data.mint_end_at) }}</span>
                </p>
              </li>
            </ul>
            <!-- 按钮 -->
            <div class="w-full flex mt-5 justify-between">
              <v-router v-if="data.tutorial_blog_url" class="block h-11 bg-global-white border-1 border-global-primary rounded-md py-1.5 px-3 flex items-center justify-center" :class="(data.participation_url || data.website) && data.mint_status !== Progress.finished ? 'w-42.5' : 'w-87'" :href="data.tutorial_blog_url" target="_blank">
                <span class="text-kd16px22px text-global-darkblue font-medium font-kdFang">{{ i18n.dapp.detail.tutorial }}</span>
              </v-router>
              <v-router v-if="(data.participation_url || data.website) && data.mint_status !== Progress.finished" class="block h-11 bg-global-primary rounded-md py-1.5 px-3 flex items-center justify-center" :class="data.tutorial_blog_url ? 'w-42.5' : 'w-87'" :href="data.participation_url || data.website" target="_blank">
