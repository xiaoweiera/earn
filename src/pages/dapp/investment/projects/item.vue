    ...
    <v-router :href="`${config.funds}/${data.project.id}`" target="_blank" name="div">
      ...
      <div class="mt-3">
        <div class="w-24 h-24 mx-auto relative">
          <div class="h-full">
            <ui-image class="h-full text-24-28" rounded fit="cover" :src="data.project.logo" :title="data.project.name"></ui-image>
          </div>
      ...
      <!-- 参与logo -->
      <div :class="{ overlap: data.investors.length > 9 }" class="mt-2 px-1.5 group-list">
        <div class="min-h-11.5 py-2 pl-1.5 border-t-1 border-b-1 border-global-highTitle border-opacity-6 flex justify-center items-center">
          <template v-for="(item, index) in data.investors" :key="item.id">
            <v-router class="block p-0.5 group-item bg-global-white rounded-1/2" :style="`z-index:${index}`" :href="`${config.invest}/${item.id}`" target="_blank">
              <ui-image class="w-6 h-6 font-kdInter text-kd12px16px text-global-white" rounded fit="cover" :src="item.logo" :title="item.name" />
            </v-router>
          </template>
