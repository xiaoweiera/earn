...
<template>
  <div class="info">
    ...
    <div v-if="data.length || loading" class="showX">
      <table class="table-my my-4">
        ...
        <tr class="border-tb">
          ...
          <td>
            <uiSort @change="sort" class="sort" :sortData="params" keyName="name" name="Project Name"/>
          </td>
          <td>
            <uiSort @change="sort" class="sort justify-center" :sortData="params" keyName="type" name="Type"/>
          </td>
          <td>
            <uiSort @change="sort" class="sort justify-center" :sortData="params" keyName="round" name="Round"/>
          </td>
          <td>
            <uiSort @change="sort" class="sort justify-end" :sortData="params" keyName="money" name="投资金额"/>
          </td>
          <td>
            <uiSort @change="sort" class="sort justify-end" :sortData="params" keyName="date" name="日期"/>
          </td>
          <td>
            <uiSort @change="sort" class="sort justify-end pr-3" :sortData="params" keyName="state" name="项目状态"/>
          </td>
        </tr>
        </thead>
        <tbody>
          ...
          <tr class="h-14">
            ...
            <td>
              <div class="flex items-center max-w-28 whitespace-nowrap">
                ...
                <div class="ml-1.5">
                  <div class="numberDefault text-number line-height-no smallTxt max-w-28 whitespace-nowrap">Astar
                    Network
                  </div>
    ...
    <div v-if="data.length > 0 && resultNumber >= params.page_size" class="more" @click="more">{{ i18n.home.loadingMore
      }}
    </div>
