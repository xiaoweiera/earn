...
onMounted(() => {
  track.push(track.Origin.gio, track.event.rank.nft);
});
</script>
<template>
      ...
      <div class="jian pb-6">
        ...
        <div class="lg:flex lg:flex-row max-w-360 mx-auto md:px-20 items-end flex-col items-stretch px-4 items-center">
          <div class="flex-1 w-full">
            <Info class="is-web flex justify-center" :title="i18n.nft.rank.title" />
            <div class="w-full flex justify-center flex-wrap mt-4 md:mt-8">
              <Chains :page="chainPage.nft_rank" :base-url="routerConfig.rankNft" />
            </div>
          </div>
        </div>
      </div>
...
.jian {
  background-image: linear-gradient(white, #fafbfc);
}
</style>
