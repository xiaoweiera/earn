...
const detailUrl = function (data: object) {
  ...
  if (id) {
    if (props.category && props.category === "defi") {
      return `${routerConfig.rankDefi}/${id}`;
    }
    if (props.category && props.category === "game") {
      return `${routerConfig.rankGame}/${id}`;
    }
    return `${routerConfig.rankDapp}/${id}`;
  }
